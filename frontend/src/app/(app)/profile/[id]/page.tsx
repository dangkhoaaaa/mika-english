      ...
      {!loading && !error ? (
        <>
          <section className="mb-4 rounded-xl border border-white/10 bg-[#242526] p-5">
              ...
              <div className="px-4 pb-3 pt-2">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-zinc-500">{maskEmailHideFirst5(data?.user?.email)}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-4 grid gap-2 sm:grid-cols-3">
              <div className="rounded-lg bg-[#18191a] p-3 text-sm text-zinc-300">
                Streak: <strong className="text-white">{data?.stats?.streakDays ?? 0}</strong>{" "}
                ngày
              </div>
              <div className="rounded-lg bg-[#18191a] p-3 text-sm text-zinc-300">
                Cá unique:{" "}
                <strong className="text-white">{data?.fishSummary?.totalUnique ?? 0}</strong>
              </div>
              <div className="rounded-lg bg-[#18191a] p-3 text-sm text-zinc-300">
                Tổng cá:{" "}
                <strong className="text-white">{data?.fishSummary?.totalCatches ?? 0}</strong>
              </div>
            </div>
          ...
          <section className="rounded-xl border border-white/10 bg-[#242526] p-5">
            <h2 className="mb-3 text-lg font-semibold text-white">Bài đăng của người này</h2>
            {(data?.posts ?? []).length === 0 ? (
              <p className="text-zinc-500">Chưa có bài đăng.</p>
            ) : (
