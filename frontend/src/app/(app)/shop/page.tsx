    ...
    <div className="mx-auto max-w-3xl px-3 py-8 sm:px-4">
      ...
      <section className="mb-5 rounded-xl border border-white/10 bg-[#242526] p-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="text-xs text-zinc-500">Cần đang dùng: {stats?.rod ?? "basic"}</div>
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 gap-3">
        {rods.map((r) => {
          ...
          return (
            <article key={r.id} className="rounded-xl border border-white/10 bg-[#242526] p-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs text-zinc-500">
                    ...
                    <strong className="text-zinc-200">{Math.round((1 - r.biteFactor) * 100)}%</strong> · Bảo hộ timing:{" "}
                    <strong className="text-zinc-200">+{Math.round(r.windowBonus * 100)}%</strong>
                  </p>
                  {owned && !active ? (
                ...
                {owned ? (
                  <button
                    ...
                    className="rounded-lg border border-white/20 bg-black/25 px-4 py-2 text-sm font-semibold text-zinc-100 hover:bg-white/5 disabled:opacity-50"
                  >
